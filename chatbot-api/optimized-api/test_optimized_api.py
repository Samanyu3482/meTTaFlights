#!/usr/bin/env python3
"""
Test script for the Optimized Flight Search API
"""
...
def test_health_check():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/api/optimized/health")
...
def test_optimized_flight_search():
    """Test the optimized flight search endpoint"""
    print("\nTesting optimized flight search...")
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/api/optimized/search",
            json=test_request,
        ...
        if response.status_code == 200:
            data = response.json()
            if data["success"]:
                flight = data["optimized_flight"]
                print(f"✅ Optimized flight found!")
...
def test_get_airlines():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/api/optimized/airlines")
        if response.status_code == 200:
...
def test_get_routes():
    ...
    try:
        response = requests.get(f"{API_BASE_URL}/api/optimized/routes")
        if response.status_code == 200:
...
def main():
    """Run all tests"""
    print("🚀 Optimized Flight API Test Suite")
    print("=" * 50)
    
    tests = [
        ("Health Check", test_health_check),
        ("Optimized Flight Search", test_optimized_flight_search),
    ...
    if passed == total:
        print("🎉 All tests passed! The Optimized Flight API is working correctly.")
    else:
