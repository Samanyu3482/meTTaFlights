...
def create_connection_flight(outbound_flight: dict, inbound_flight: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            "airlines": [outbound_flight['airline'], inbound_flight['airline']],  # NEW: Combined airlines
            "segments": [
                {
                    "source": outbound_flight['source'],
                    ...
                    "airline": outbound_flight['airline']  # NEW: Airline for segment
                },
                {
                    ...
                    "airline": inbound_flight['airline']  # NEW: Airline for segment
                }
            ]
        }
    except Exception as e:
...
def find_connecting_flights(source: str, destination: str, year: int, month: int, day: int, priority: str = "cost", airline: str = None) -> list:
    ...
    """
    try:
        # Use MeTTa to get all flights for the given date - Updated for airline parameter
...
def metta_serializer(metta_result):
    """Updated serializer to handle airline parameter"""
    result = []
    if not metta_result:
        return result
    ...
    for item in data_to_process:
        if isinstance(item, ExpressionAtom):
            ...
            if len(expr) >= 10 and str(expr[0]) == "flight":  # NEW: Now has 10 parts (added airline)
                takeoff_time = str(expr[7])
                landing_time = str(expr[8])
                airline = str(expr[9])  # NEW: Airline parameter
                ...
                result.append({
                    ...
                    "airline": airline,  # NEW: Added airline
                    "duration": duration
                })
            ...
            if item_str.startswith("(flight "):
                ...
                if len(parts) >= 10:  # NEW: Now has 10 parts (added airline)
                    takeoff_time = parts[7]
                    landing_time = parts[8]
                    airline = parts[9]  # NEW: Airline parameter
                    duration = calculate_flight_duration(takeoff_time, landing_time)
...
def search_flights(source=None, destination=None, year=None, month=None, day=None, airline=None, priority="cost"):
    """Updated search flights with airline parameter"""
    ...
    airline_pattern = airline if airline else "$airline"  # NEW: Airline pattern
    
    # Updated query with airline parameter
    query = f'''!(match &space 
...
# Updated search functions with airline parameter
def search_by_source(source, airline=None, priority="cost"):
    """Search flights by source airport only"""
    return search_flights(source=source, airline=airline, priority=priority)

def search_by_destination(destination, airline=None, priority="cost"):
    """Search flights by destination airport only"""
    return search_flights(destination=destination, airline=airline, priority=priority)

def search_by_date(year, month, day, airline=None, priority="cost"):
    """Search flights by date only"""
    return search_flights(year=year, month=month, day=day, airline=airline, priority=priority)

def search_by_route(source, destination, airline=None, priority="cost"):
    """Search flights by source and destination"""
    return search_flights(source=source, destination=destination, airline=airline, priority=priority)

def search_by_airline(airline, priority="cost"):
    """NEW: Search flights by airline only"""
    return search_flights(airline=airline, priority=priority)

def search_by_source_date(source, year, month, day, airline=None, priority="cost"):
    """Search flights by source and date"""
    return search_flights(source=source, year=year, month=month, day=day, airline=airline, priority=priority)

def search_by_dest_date(destination, year, month, day, airline=None, priority="cost"):
    """Search flights by destination and date"""
    return search_flights(destination=destination, year=year, month=month, day=day, airline=airline, priority=priority)

def search_comprehensive(source, destination, year, month, day, airline=None, priority="cost"):
    """Search flights by all parameters"""
    return search_flights(source=source, destination=destination, year=year, month=month, day=day, airline=airline, priority=priority)

def search_all_flights(airline=None, priority="cost"):
    """Get all flights"""
    return search_flights(airline=airline, priority=priority)
...
def smart_search(source=None, destination=None, year=None, month=None, day=None, airline=None, priority="cost", include_connections=True):
    """
    Updated smart search with airline parameter
    """
...
def get_user_input_and_search():
    """User input function - shows all airlines by default"""
    ...
    # Removed airline input - will search all airlines by default
    ...
    airline = None  # Always None - searches all airlines
    
    return smart_search(source, destination, year, month, day, airline)

def display_flight_info(flights):
    """Helper function to display flight information nicely"""
    if not flights:
    ...
    for i, flight in enumerate(flights, 1):
        print(f"Flight {i}:")
        print(f"  Date: {flight['year']}-{flight['month']}-{flight['day']}")
        print(f"  Route: {flight['source']} → {flight['destination']}")
        print(f"  Airline: {flight['airline'].replace('_', ' ')}")  # Replace underscores with spaces
        print(f"  Departure: {flight['takeoff']}")  # Show original format
        print(f"  Arrival: {flight['landing']}")    # Show original format
        print(f"  Duration: {flight['duration']} minutes")
        print(f"  Cost: ${flight['cost']}")
        
        # Display connection info if it's a connecting flight
        if flight.get('is_connecting'):
            print(f"  Connection: via {flight['connection_airport']}")
            print(f"  Layover: {flight['layover_hours']} hours")
            print(f"  Airlines: {' + '.join([airline.replace('_', ' ') for airline in flight['airlines']])}")
...
def get_clean_flight_data(flights):
    """Return clean list of dictionaries with flight data only"""
    clean_flights = []
    ...
    for flight in flights:
        clean_flight = {
            ...
            "arrival": flight['landing'],
            "airline": flight['airline'].replace('_', ' '),
            "duration_minutes": flight['duration']
        }
        
        # Add connection info if it's a connecting flight
        if flight.get('is_connecting'):
            clean_flight.update({
                "is_connecting": True,
                "connection_airport": flight['connection_airport'],
                "layover_hours": flight['layover_hours'],
                "airlines": [airline.replace('_', ' ') for airline in flight['airlines']],
                "segments": flight['segments']
            })
        else:
            clean_flight["is_connecting"] = False
...
if __name__ == "__main__":
    ...
    # Option 1: Get formatted display (current behavior)
    display_flight_info(flights)
    
    # Option 2: Get clean list of dictionaries  
